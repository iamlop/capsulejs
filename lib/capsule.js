...
capsule.prototype.deploy = function(target) {
    ...
    var text_status = 'Start deploy ' + target;
    console.log(text_status.green);
    ssh
        .exec('mkdir', {
...
capsule.prototype.postCommand = function(server, params) {
    ...
    async.forEachOf(params.post_command, function (cmd, key, callback) {
        var cmd_with_pull_path = cmd.replace(/{dir}/g, cmd);
            ssh
                .exec(cmd_with_pull_path, {
                    out: function(stdout) {
                        console.log(util.format('%s : %s', key, stdout));
                    }
                });
    });
    ssh.start();
