...
module.exports = class Capsule {
  ...
  init(config = null) {
    config = (typeof(config) === 'string') ? config : 'capsule.json'
    let targetFile = process.cwd() + '/' + config
    let sourceFile = __dirname + '/../templates/capsule.json'

    if (fs.existsSync(targetFile)) {
      console.log(`File ${config} exist`.red)
    } else {
  ...
  async parseConfig(target, configName = null) {
    return new Promise(async function (resolve, reject) {
      configName = (typeof(configName) === 'string') ? configName : 'capsule.json'
      let configFilePath = process.cwd() + '/' + configName
      let config = null
      let params = {}

      if (fs.existsSync(configFilePath) == false) {
        reject('Error config file not exist')
        return
      }
      ...
      if (typeof config[target] === 'undefined') {
        reject('Error undefined server')
        return
      }

      const formatDir = dayjs().format('YYYY-MM-DD_HH-mm-ss')
  ...
  async deploy(target, ip = [], config = null) {
    let params = null
    let capsule = this
    let textMsg = null
    let output = null
    ...
    try {
      params = await this.parseConfig(target, config)
    } catch(e) {
    ...
    async.eachSeries(currentServer, async function (s) {
      ...
      console.log('Cloning git: ' + 'START'.yellow)
  ...
  async rollback(target, config = null) {
    let params = await this.parseConfig(target, config)
