...
capsule.prototype.deploy = function(target) {
    ...
    ssh
        ...
        .exec('git clone', {
            ...
            out: console.log,
            exit: function() {
                capsule.simlink(ssh, params.server, params.full_path);
                capsule.postCommand(ssh, params.full_path, params.post_command);
            }
        })
        .start();
};

capsule.prototype.simlink = function(ssh, server, destination) {
    ssh
        .exec('ln -nfs', {
...
capsule.prototype.postCommand = function(ssh, realpath, commands) {
    ...
    ssh
        .exec('ls', {
            out: function(lists) {
                ...
                capsule.simlink(ssh, params.server, prev);
                capsule.deleteDir(ssh, current);
            }
        })
        .start();
};

capsule.prototype.deleteDir = function(ssh, target) {
    ssh
        .exec('rm -rf', {
            args: [target],
            out: console.log
